...
# Directory for Saving Checkpoints of Word Embedding Layer
word_embd_dir = 'checkpoint/word_embd'
# Directory for Saving Checkpoints of Model
model_dir = 'checkpoint/model3v2'
# Directory for Saving Summaries of training
summary_dir = model_dir + '/summary'
word_embd_dim = 100                     # Dimension of embedding layer for words
pos_embd_dim = 25                       # Dimension of embedding layer for POS Tags
# Dimension of embedding layer for Dependency Types
dep_embd_dim = 25
...
# Maximum no. of childrens in Dependency Tree
max_num_child = 20

lambda_l2 = 0.0005                        # lambda of l2-regulaizer
...
with tf.compat.v1.name_scope("input"):
    ...
    sp_childs = tf.compat.v1.placeholder(
        tf.int32, [batch_size, 2, max_len_path, max_num_child], name="sp_childs")
...
def lstm_dep_init(channel, dep_input_size, state_size):
    ...
    with tf.compat.v1.variable_scope(channel):
        ...
        U_ffp = tf.compat.v1.get_variable("U_ffp", shape=[max_num_child, state_size, state_size], initializer=tf.compat.v1.keras.initializers.VarianceScaling(
            scale=1.0, mode="fan_avg", distribution="uniform"))
...
with tf.compat.v1.name_scope("dropout_hidden_seq"):
    y_hidden_layer_drop = []
    for i in range(batch_size):
        y_drop = tf.nn.dropout(y_hidden_layer[i], rate=1 - (0.5))
        y_hidden_layer_drop.append(y_drop)

# SoftMax Layer for Sequencesnipping
...
with tf.compat.v1.name_scope("dropout_hidden_dep"):
    y_p_drop = tf.nn.dropout(y_p, rate=1 - (0.5))
...
for g, v in grads_vars:
    if g is not None:
        grad_hist_summary = tf.compat.v1.summary.histogram(
            "{}/grad/hist".format(v.name), g)
        sparsity_summary = tf.compat.v1.summary.scalar(
            "{}/grad/sparsity".format(v.name), tf.nn.zero_fraction(g))
        grad_summaries.append(grad_hist_summary)
        grad_summaries.append(sparsity_summary)
grad_summaries_merged = tf.compat.v1.summary.merge(grad_summaries)

loss_seq_summary = tf.compat.v1.summary.scalar("loss_seq", loss_seq)
loss_dep_summary = tf.compat.v1.summary.scalar("loss_dep", loss_dep)
total_loss_summary = tf.compat.v1.summary.scalar("total_loss", total_loss)
...
#         _, _loss, step, _summary = sess.run([optimizer_seq, loss_seq, global_step_seq, summary], feed_dict)
    ...
    # For complete model training
        _, _loss, step, _summary = sess.run(
            [train_op, total_loss, global_step_dep, summary], feed_dict)
